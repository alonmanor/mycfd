...
!############################################################################
!visualization for LES
!############################################################################
! eddy viscousity
if ((iles.eq.1).or.(iles.eq.2)) then
	uvisu=0.
	call fine_to_coarseV(1,xnu_sgs1,uvisu)
997 format('nu_sgs',I3.3)
...
!############################################################################
!
subroutine visu_dissipation (ux1,uy1,uz1,phi1,ta1,tb1,tc1,td1,te1,tf1,tg1,th1,ti1,di1,&
     ta2,tb2,tc2,td2,te2,tf2,tg2,th2,ti2,tj2,di2,&
     ta3,tb3,tc3,td3,te3,tf3,tg3,th3,ti3,di3,phG,uvisu,k_sgs1,xnu_sgs1,&
     evec_x1,evec_y1,evec_z1)
!
...
!############################################################################
!energy dissipation (tau_ij S_ij)
di1=0.
do i=1,xsize(1)
do j=1,xsize(2)
do k=1,xsize(3)
   di1(i,j,k)=(0.0-evec_x1(i,j,k)*evec_x1(i,j,k))*ta1(i,j,k)
   di1(i,j,k)=(   -evec_x1(i,j,k)*evec_y1(i,j,k))*td1(i,j,k)+di1(i,j,k)
   di1(i,j,k)=(   -evec_x1(i,j,k)*evec_z1(i,j,k))*tg1(i,j,k)+di1(i,j,k)
   di1(i,j,k)=(   -evec_y1(i,j,k)*evec_x1(i,j,k))*tb1(i,j,k)+di1(i,j,k)
   di1(i,j,k)=(0.0-evec_y1(i,j,k)*evec_y1(i,j,k))*te1(i,j,k)+di1(i,j,k)
   di1(i,j,k)=(   -evec_y1(i,j,k)*evec_z1(i,j,k))*th1(i,j,k)+di1(i,j,k)
   di1(i,j,k)=(   -evec_z1(i,j,k)*evec_x1(i,j,k))*tc1(i,j,k)+di1(i,j,k)
   di1(i,j,k)=(   -evec_z1(i,j,k)*evec_y1(i,j,k))*tf1(i,j,k)+di1(i,j,k)
   di1(i,j,k)=(0.0-evec_z1(i,j,k)*evec_z1(i,j,k))*ti1(i,j,k)+di1(i,j,k)
!~    if (di1(i,j,k) < 0.0.and.nrank.eq.0) then
!~    print *,evec_x1(i,j,k), evec_y1(i,j,k), evec_z1(i,j,k)
!~    print *,ta1(i,j,k),td1(i,j,k),tg1(i,j,k)
!~    print *,tb1(i,j,k),te1(i,j,k),th1(i,j,k)
!~    print *,tc1(i,j,k),tf1(i,j,k),ti1(i,j,k)
!~    print *,i-1+xstart(1),j-1+xstart(2),k-1+xstart(3)
!~    stop
!~    endif
   di1(i,j,k)=di1(i,j,k)*k_sgs1(i,j,k)
enddo
enddo  
enddo
!~ stop
uvisu=0.
call fine_to_coarseV(1,di1,uvisu)
990 format('diss',I3.3)
write(filename, 990) itime/imodulo
call decomp_2d_write_one(1,uvisu,filename,2)
!call decomp_2d_write_one(nx_global,ny_global,nz_global,&
!     1,di1,filename)
!############################################################################
end subroutine visu_dissipation
